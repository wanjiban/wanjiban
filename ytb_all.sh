...
if [ -z "$PLAYLISTS" ]; then
    echo "无法获取播放列表，检查用户名是否正确或网络连接问题。"
    echo "将下载 ${USERNAME} 频道下的所有视频。"

    # 下载用户频道下的所有视频
    $YT_DLP -o "${USER_DIR}/%(upload_date)s - %(title)s.%(ext)s" "https://www.youtube.com/${USERNAME}/videos"
    ...
    echo "频道 ${USERNAME} 下的所有视频已下载到目录 ${USER_DIR}"
    exit 0
fi
